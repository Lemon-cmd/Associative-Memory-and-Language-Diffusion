#!/bin/bash
#SBATCH -J duo-owt-small                    # Job name
#SBATCH -o watch_folder/duo_owt_small/%x_%j.out     # output file (%j expands to jobID)
...
#SBATCH --ntasks-per-node=8           # Processes per compute node
#SBATCH --gres=nvme,gpu:32g:8         # Type/number of GPUs needed
...
srun python -u -m main \
  ...
  wandb.name=duo-owt-${subset}-${size} \
  wandb.project=owt-unpack-${size} \
  model=${size} \
  algo=duo \
  model.length=1024 \
