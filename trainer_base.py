...
def sample_categorical(categorical_probs, deterministic=False):
    if deterministic:
        # Greedy sampling
        # p_i = argmax_i (c_i) where c_i are the categorical probabilities and we do not add noise
        return categorical_probs.argmax(dim=-1)

    # Stochastic sampling using the Gumbel-max trick
    # p_i = argmax_i (log(c_i) + g_i) where g_i ~ Gumbel(0,1) which we can interpret as adding noise to the log-probs
...
class TrainerBase(L.LightningModule):
    def __init__(
        ...
        self.deterministic_posterior = self.config.sampling.get(
    ...
    def training_step(self, batch, batch_idx):
        # [ADD] Reset stats at the start of the step to get accurate per-step readings
        ...
        # [ADD] Log peak memory usage for this specific step
        peak_mem_gb = torch.cuda.max_memory_allocated() / (1024**3)
        self.log("perf/peak_mem_gb", peak_mem_gb, on_step=True, prog_bar=True)
...
class AbsorbingState(Diffusion):
    ...
    def _ancestral_update(self, x, t, dt, p_x0=None, noise_removal_step=False):
        ...
        _x = sample_categorical(q_xs, deterministic=self.deterministic_posterior)
    ...
    def _analytic_update(self, x, t, dt):
        ...
        return sample_categorical(probs, deterministic=self.deterministic_posterior)
    ...
    def _denoiser_update(self, x, t):
        ...
        samples = sample_categorical(probs, deterministic=self.deterministic_posterior)
        return samples
