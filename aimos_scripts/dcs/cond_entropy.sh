...
TIMES=(0.00001 0.05 0.1 0.25 0.5 1.0)

# 1. Convert Bash Array to Hydra-formatted String "[t1,t2,t3]"
# We join the array with commas
...
for subset in "${SUBSET_SIZES[@]}"; do
    ...
    if [[ -f "$ckpt_path" ]]; then
        ...
        sbatch <<EOT
#!/bin/bash
#SBATCH --job-name=entropy-${data}-${size}-${subset}
#SBATCH --error=watch_folder/dcs-errs/entropy/${data}-${size}/%x_%j.err
#SBATCH --output=watch_folder/dcs-outs/entropy/${data}-${size}/%x_%j.out
#SBATCH --nodes=${num_nodes}
#SBATCH --gres=nvme,gpu:32g:${num_gpus}
#SBATCH --ntasks-per-node=${num_gpus}
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --time=01:00:00  # Increased time since we are doing multiple t's
#SBATCH --partition=dcs-2024

export TRANSFORMERS_OFFLINE=1
export HF_DATASETS_OFFLINE=1

srun --export=ALL python -u -m eval_entropy_multi \\
      hydra.run.dir=${work_dir}/experiments/entropy/"${data}-${size}"/${subset} \\
      mode=sample_eval \\
      data=${data} \\
      model=${size} \\
      algo=duo \\
      ...
      +eval.synthetic_data_path=${work_dir}/synthetic_data/"${data}-${size}"/${subset}/synthetic_data/
EOT
    else
