...
#SBATCH --error=watch_folder/dcs-errs/stability/${data}-${size}/%x_%j.err
#SBATCH --output=watch_folder/dcs-outs/stability/${data}-${size}/%x_%j.out
...
srun --export=ALL python -u -m eval_fixed_point_multi \
      hydra.run.dir=${work_dir}/experiments/stability/"${data}-${size}"/${subset} \
      mode=sample_eval \
      data=${data} \
      model=${size} \
      algo=duo \
