...
class StabilityEvaluator(BasinEvaluator):
    ...
    def run_eval_loop(self, evalset, samples_to_process, batch_size, eval_time):
        ...
        for i in range(0, len(indices), batch_size):
            ...
            if one_time_print:
                self.logger.info(
                    f"[Eval t={eval_time}] Batch Size: {current_bs} | Time: {time()-st:.4f}s"
                )
...
def main(config):
    ...
    # --- 5. Evaluation Loop over Times ---
    if rank == 0:
        evaluator.logger.info(f"Target Samples per GPU: {samples_per_gpu}")
        evaluator.logger.info(f"Evaluating for times: {eval_times}")

    for eval_time in eval_times:
        output_csv_path = f"entropy_gap_t={eval_time}.csv"
        output_npz_path = f"entropy_gap_t={eval_time}.npz"

        # Check existence to skip redundant work
        if os.path.exists(output_csv_path) and not override:
            if rank == 0:
                evaluator.logger.info(
                    f"File {output_csv_path} exists. Skipping t={eval_time}."
                )
            dist.barrier()  # Ensure all ranks wait before continuing to next t
            continue

        if rank == 0:
            evaluator.logger.info(f"--- Processing t={eval_time} ---")
        ...
        # --- 6. Gather & Save Results (Per Time Step) ---
        ...
        if rank == 0:
            ...
            evaluator.logger.info(
                f"Real Entropy: {avg_real:.4f} | Synth Entropy: {avg_synth:.4f}"
            )
            evaluator.logger.info(f"CONDITIONAL ENTROPY GAP: {avg_entropy_gap:.4f}")

            # Save per-timestep file
            my_dict = dict(
                {
                    "entropy": np.concatenate([real_np.mean(1), synth_np.mean(1)]),
            ...
            evaluator.logger.info(f"Saved results to {output_csv_path}")

        # Barrier to ensure Rank 0 finishes saving before other ranks potentially exit or start next loop
        dist.barrier()
