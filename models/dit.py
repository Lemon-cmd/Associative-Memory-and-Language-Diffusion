...
try:
    ...
    print("Flash Attention 2 not found. Falling back to xFormers/PyTorch.")
    try:
...
def apply_rotary_emb_torch(x, cos, sin):
    """
    Manual fallback for applying rotary embeddings.
    Expects x: (batch, seq, head, dim) or similar
    """
    # Simple view change for complex number multiplication
    # Assumes dim is last
    head_dim = x.shape[-1]
    x1 = x[..., : head_dim // 2]
    x2 = x[..., head_dim // 2 :]

    # Standard rotary formula: [-x2, x1] * sin + [x1, x2] * cos
    return torch.cat((-x2, x1), dim=-1) * sin + x * cos


def apply_rotary_pos_emb(qkv, cos, sin):
    # This slicing reduces dim from 64 -> 32
    ...
    if USE_FLASH_ATTN:
        ...
        # --- FALLBACK FIX ---
        # 1. Repeat back to 64: (Seq, 32) -> (Seq, 64)
        cos = torch.cat([cos, cos], dim=-1)
        sin = torch.cat([sin, sin], dim=-1)

        # 2. Add broadcasting dims: (Seq, 64) -> (1, Seq, 1, 64)
        # Matches qkv: (Batch, Seq, Heads, Dim)
        cos = cos[None, :, None, :]
        sin = sin[None, :, None, :]

        # 3. Apply manual rotary
        q, k, v = qkv.unbind(2)
...
def split_and_apply_rotary_pos_emb(qkv, rotary_cos_sin):
    ...
    if USE_FLASH_ATTN:
        # Flash Attn handles the slicing natively
        q = flash_attn.layers.rotary.apply_rotary_emb_torch(q, cos, sin)
        k = flash_attn.layers.rotary.apply_rotary_emb_torch(k, cos, sin)
    else:
        # --- FALLBACK FIX ---
        # 1. Repeat back to dim: (Seq, 32) -> (Seq, 64)
        # This assumes head_dim is 64
        cos = torch.cat([cos, cos], dim=-1)
        sin = torch.cat([sin, sin], dim=-1)
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        # Calculate in float32 for numerical stability, then cast back to input dtype
        orig_dtype = x.dtype
        x = F.layer_norm(x.float(), [self.dim]).to(orig_dtype)
...
class DDiTBlockCausal(nn.Module):
    ...
    def forward(self, x, rotary_cos_sin, **kwargs):
        ...
        # Apply Rotary
        cos, sin = rotary_cos_sin
        # This calls our wrapper from Step 2
        ...
        if USE_FLASH_ATTN:
            ...
            # XFORMERS FALLBACK (V100 Compatible)
            # qkv is (B, S, 3, H, D). xFormers wants (B, S, H, D)
            q, k, v = qkv.unbind(2)

            # xFormers Memory Efficient Attention
            # Note: xFormers on V100 prefers (B, S, H, D) or (B, H, S, D) depending on version,
            # but modern xops.memory_efficient_attention handles (B, S, H, D) natively.
            x = xops.memory_efficient_attention(
                q,
                k,
                v,
                attn_bias=xops.LowerTriangularMask(),  # This enables Causal masking
            )
            x = einops.rearrange(x, "b s h d -> b s (h d)")

        else:
            # PYTORCH 2.0 SDPA FALLBACK (If xFormers fails)
            q, k, v = qkv.unbind(2)
            # SDPA expects (B, H, S, D)
            q, k, v = map(lambda z: z.transpose(1, 2), (q, k, v))
...
class DIT(nn.Module, huggingface_hub.PyTorchModelHubMixin):
    ...
    def forward(self, x, sigma):
        ...
        with torch.amp.autocast('cuda', dtype=self._get_autocast_dtype()):
