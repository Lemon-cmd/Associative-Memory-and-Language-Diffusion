...
def get_dataloaders(
    ...
    if (
        ...
        % (config.trainer.devices * config.trainer.accumulate_grad_batches)
        != 0
    ):
        raise ValueError(
            f"Train Batch Size {config.training.batch_size}"
            f"not divisible by {config.trainer.devices} gpus with accumulation "
            f"{config.trainer.accumulate_grad_batches}."
        )
    if config.loader.eval_global_batch_size % config.trainer.devices != 0:
        raise ValueError(
            f"Eval Batch Size for {config.eval.batch_size} "
            f"not divisible by {config.trainer.devices}."
        )
    if skip_train:
        train_set = None
    else:
    ...
    # === NEW SPLITTING LOGIC (Supports Fraction OR Absolute Count) ===
    subset_val = config.data.get("subset", None)

    if subset_val is not None and subset_val > 0:
        if config.data.streaming:
            # Streaming logic: If absolute, take N. If fraction, estimate N (rough).
            limit = int(subset_val) if subset_val > 1.0 else int(subset_val * 1000000)
            LOGGER.warning(f"Streaming mode subset: Taking first {limit} samples.")
            train_set = train_set.take(limit)

        else:
            total_samples = len(train_set)

            # Logic: If <= 1.0, treat as a Ratio (e.g. 0.5 = 50%)
            #        If > 1.0, treat as Absolute Number (e.g. 10000 = 10k samples)
            if subset_val <= 1.0:
                subset_samples = int(total_samples * subset_val)
                mode_desc = f"Fraction: {subset_val} and |D| = {subset_samples}"
            else:
                subset_samples = int(subset_val)
                mode_desc = f"Absolute: {subset_samples}"

            # Safety Check: Don't crash if requested > total
            if subset_samples > total_samples:
                subset_samples = total_samples

            LOGGER.info(
                f"Subsampling Training Set: {subset_samples}/{total_samples} samples ({mode_desc})"
            )

            # Deterministically shuffle with fixed seed, then take the first N
            # This ensures all GPUs agree on WHICH 10,000 samples to use
            train_set = train_set.shuffle(seed=config.seed).select(
                range(subset_samples)
            )
    # =================================================================
